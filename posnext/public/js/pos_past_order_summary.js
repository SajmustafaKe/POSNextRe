...
posnext.PointOfSale.PastOrderSummary = class {
    ...
    split_order() {
        ...
        if (this.doc.items.length < 2) {
            frappe.msgprint(__('The invoice must have at least two items to split.'));
            return;
        }
    ...
    process_split_invoices(dialog) {
        const items_table = dialog.get_value('items_table');
        if (!items_table || !items_table.length) {
            frappe.msgprint(__('No items selected for splitting.'));
            return;
        }
        ...
        group_keys.forEach((group, idx) => {
            ...
            if (idx === 0) {
                ...
                remaining_items.push(...group_items);
            } else {
                // Create new invoice for other groups
                promises.push(
                    frappe.call({
                        ...
                        args: {
                            doc: {
                                ...
                                items: group_items.map(item => ({ ...item })),
            ...
            promises.push(
                frappe.call({
                    ...
                    args: {
                        ...
                        name: this.doc.name,
                        fieldname: 'items',
                        value: remaining_items.map(item => ({ ...item }))
                    }
                })
            );
        } else {
        ...
        Promise.all(promises)
            .then(results => {
                ...
                dialog.hide();
                frappe.show_alert({
                    message: __('Invoices split successfully.'),
                    indicator: 'green'
                });
                ...
                if (remaining_items.length > 0) {
                    frappe.call({
                        ...
                        callback: r => {
                            if (!r.exc) {
                                this.events.load_summary_of(r.message);
                            }
                        }
                    });
                } else {
            ...
            frappe.call({
                ...
                callback: (r) => {
                    if (!r.exc && r.message) {
                        ...
                        console.log("Items to render:", doc_data.items);
                        try {
