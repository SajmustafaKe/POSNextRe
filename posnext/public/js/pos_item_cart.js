...
frappe.throw = function(msg, exception_type) {
    ...
    if (typeof msg === 'string') {
        // Pattern 1: "Sales Invoice new-sales-invoice-xxxxx not found"
        if (msg.includes('Sales Invoice') && 
            msg.includes('not found') && 
            msg.includes('new-sales-invoice-')) {
            
            console.log('Suppressing Sales Invoice not found error (Pattern 1):', msg);
        ...
        // Pattern 2: Just "Sales Invoice xxxxx not found" (without "new-sales-invoice-")
        if (msg.match(/Sales Invoice .+ not found/)) {
            console.log('Suppressing Sales Invoice not found error (Pattern 2):', msg);
        ...
        // Pattern 3: Any temporary invoice reference error
        if (msg.includes('not found') && 
            (msg.includes('new-sales-invoice-') || msg.includes('new-pos-invoice-'))) {
            
            console.log('Suppressing temporary invoice not found error (Pattern 3):', msg);
...
// ADDITIONAL: Also override frappe.msgprint to catch these errors there too
const original_frappe_msgprint = frappe.msgprint;
frappe.msgprint = function(msg, title, indicator) {
    // Check if this is a "Sales Invoice not found" error
    if (typeof msg === 'string') {
        if ((msg.includes('Sales Invoice') && msg.includes('not found')) ||
            (msg.includes('not found') && msg.includes('new-sales-invoice-'))) {
            
            console.log('Suppressing Sales Invoice not found msgprint:', msg);
    ...
    // For all other messages, use the original frappe.msgprint
    original_frappe_msgprint.call(this, msg, title, indicator);
};

// ADDITIONAL: Override console.error to suppress these errors from console
const original_console_error = console.error;
console.error = function(...args) {
    // Check if this is the Sales Invoice not found error
    const errorMsg = args.join(' ');
    if ((errorMsg.includes('Sales Invoice') && errorMsg.includes('not found')) ||
        (errorMsg.includes('not found') && errorMsg.includes('new-sales-invoice-'))) {
        
        console.log('Suppressed Sales Invoice not found console error:', errorMsg);
