...
posnext.PointOfSale.Payment = class {
                ...
                if (backup_data.payments) {
                    backup_data.payments.forEach(backup_payment => {
                        ...
                        if (current_payment) {
                            ...
                            console.warn(`Payment method ${backup_payment.mode_of_payment} not found. Adding it.`);
                            const new_payment = {
                                mode_of_payment: backup_payment.mode_of_payment,
                                amount: backup_payment.amount,
                                type: backup_payment.type || 'Cash',
                                reference_no: backup_payment.reference_no || '',
                                remarks: backup_payment.remarks || ''
                            };
                            frappe.model.add_child(current_doc, 'payments', new_payment);
                        }
                    });
                }
        ...
        if (!$('#split-payment-styles').length) {
            $('head').append(`
                <style id="split-payment-styles">
                    ...
                    .split-payment-toggle { display: flex; align-items: center; gap: 10px; margin-left: auto; }
    ...
    toggle_split_payment_mode(enable) {
        ...
        if (enable) {
            ...
            this.ensure_all_payment_methods_visible();
    ...
    ensure_all_payment_methods_visible() {
        const doc = this.events.get_frm().doc;
        
        console.log('🔍 Ensuring all payment methods are visible...');
        
        doc.payments.forEach((payment, index) => {
            const mode = payment.mode_of_payment.replace(/ +/g, "_").toLowerCase();
            const wrapper = this.$payment_modes.find(`[data-mode="${mode}"]`).closest('.payment-mode-wrapper');
            
            if (wrapper.length === 0) {
                console.warn(`⚠️ Payment method not found in DOM: ${payment.mode_of_payment}`);
                this.add_missing_payment_method(payment, index);
            } else {
                console.log(`✅ Payment method visible: ${payment.mode_of_payment}`);
                wrapper.show();
            }
        });
    ...
    add_missing_payment_method(payment, index) {
        const mode = payment.mode_of_payment.replace(/ +/g, "_").toLowerCase();
        const payment_type = payment.type;
        const amount = payment.amount > 0 ? format_currency(payment.amount, this.events.get_frm().doc.currency) : '';
        
        const missing_html = `
            <div class="payment-mode-wrapper">
                <div class="mode-of-payment" data-mode="${mode}" data-payment-type="${payment_type}">
                    <div class="payment-mode-header">
                        <span class="payment-mode-name">${payment.mode_of_payment}</span>
        ...
        this.$payment_modes.append(missing_html);
        ...
        console.log(`✅ Added missing payment method: ${payment.mode_of_payment}`);
    }
    ...
    check_for_existing_payments() {
        ...
        const is_edited_order = !doc.__islocal && doc.docstatus === 0;
        
        if (is_edited_order) {
            console.log('🔍 Detected existing draft being edited');
            ...
            if (total_existing_amount === 0 && doc.paid_amount > 0) {
                total_existing_amount = doc.paid_amount;
            }
            
            if (total_existing_amount > 0 && !this.is_split_mode) {
                console.log('🎯 Enabling split mode for edited order with existing payments');
                this.$component.find('#split-payment-checkbox').prop('checked', true);
                this.toggle_split_payment_mode(true);
                
                frappe.show_alert({
                    message: __("Split payment mode enabled - found existing payments: {0}", [format_currency(total_existing_amount, doc.currency)]),
                    indicator: "blue"
                });
                return;
            }
        ...
        if (has_existing_payments && total_existing_amount > 0 && !this.is_split_mode) {
            ...
            console.log('❌ No existing payments found - keeping split mode OFF');
    ...
    fetch_original_payment_data(invoice_name) {
        ...
        frappe.db.get_doc('POS Invoice', invoice_name).then(original_doc => {
            ...
            frappe.show_alert({
                message: __("Could not fetch original payment data. Please check manually."),
                indicator: "red"
            });
        });
    }
        ...
        original_doc.payments.forEach(original_payment => {
            if (original_payment.amount && original_payment.amount > 0) {
                ...
                if (current_payment) {
                    ...
                    console.warn(`Payment method ${original_payment.mode_of_payment} not found. Adding it.`);
                    const new_payment = {
                        mode_of_payment: original_payment.mode_of_payment,
                        amount: original_payment.amount,
                        type: original_payment.type || 'Cash',
                        reference_no: original_payment.reference_no || '',
                        remarks: original_payment.remarks || ''
                    };
                    frappe.model.add_child(current_doc, 'payments', new_payment);
                    total_paid += original_payment.amount;
                }
    ...
    load_existing_payments() {
        ...
        if (this.original_payment_data && Array.isArray(this.original_payment_data)) {
            console.log('🔄 Loading from original payment data');
                ...
                if (payment.amount && payment.amount > 0) {
                    ...
                    if (payment.remarks) {
                        try {
                            ...
                            console.error('Error parsing split details from remarks:', e);
                            split_details = [{
                                amount: payment.amount,
                                reference: payment.reference_no || '',
        ...
        console.log('🎨 Ensuring ALL payment methods are available for split payments...');
        ...
        } else if (this.split_payments.length > 0) {
            frappe.show_alert({
                message: __("Existing payments found ({0})", [this.split_payments.length]),
                indicator: "blue"
            });
        }
    }
                ...
                if (parts) {
                    ...
                    if (amount > 0) {
                        ...
                        const split_payment = {
                            ...
                            mode: mode,
                            mode_of_payment: display_name.replace(/ #\d+$/, ''),
    ...
    render_split_payment_modes() {
        ...
        console.log('🎨 Rendering split payment modes for ALL payment methods...');
        console.log('Available payment methods:', payments.map(p => p.mode_of_payment));
        ...
        this.$payment_modes.html(
            payments.map((p, i) => {
                ...
                return `
                    <div class="payment-mode-wrapper" data-payment-method="${p.mode_of_payment}">
                        <div class="mode-of-payment" data-mode="${mode}" data-payment-type="${payment_type}">
                            <div class="payment-mode-header">
        ...
        console.log(`✅ Rendered ${payments.length} payment methods in split mode`);
        ...
        payments.forEach(p => {
            ...
            console.log(`✅ Created control for: ${p.mode_of_payment} (Amount: ${p.amount || 0})`);
        });
        ...
        if (!$('#split-payment-modes-styles').length) {
            $('head').append(`
                <style id="split-payment-modes-styles">
                    .payment-mode-wrapper { margin-bottom: 10px; width: 100%; display: block !important; }
                    .payment-mode-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px; }
                    .payment-mode-name { font-weight: bold; color: #333; }
                    .payment-mode-amount { color: #28a745; font-weight: bold; font-size: 14px; }
                    .mode-of-payment { border: 1px solid #ddd; border-radius: 6px; padding: 10px; background-color: #f8f9fa; margin-bottom: 8px; display: block !important; }
                    .mode-of-payment:hover { background-color: #e9ecef; border-color: #adb5bd; }
                    .payment-mode-split-active { border-color: #007bff !important; background-color: #e7f3ff !important; }
                    .add-to-split-btn { width: 100%; margin-top: 8px; }
                    .mode-of-payment-control { margin: 8px 0; display: block !important; }
                    .payment-modes .payment-mode-wrapper { opacity: 1 !important; visibility: visible !important; height: auto !important; }
                </style>
            `);
        }
        
        console.log('🎨 Split payment modes rendering complete');
    }
    ...
    render_payment_section() {
        ...
        setTimeout(() => {
            ...
            if (this.is_split_mode) {
                ...
                console.log('✅ Split mode not enabled - displaying regular payment mode');
                this.$component.find('#split-payment-checkbox').prop('checked', false);
                this.render_payment_mode_dom();
            }
        }, 100);
        ...
        this.original_payment_data.forEach((payment, index) => {
            if (payment.amount && payment.amount > 0) {
                ...
                if (current_payment) {
                    ...
                    console.warn(`Payment method ${payment.mode_of_payment} not found. Adding it.`);
                    const new_payment = {
                        mode_of_payment: payment.mode_of_payment,
                        amount: payment.amount,
                        type: payment.type || 'Cash',
